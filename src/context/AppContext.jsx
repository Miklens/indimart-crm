...
function sanitizeInvoiceNumbers(history) {
  ...
  return history.map(inv => {
    ...
    if (!Array.isArray(rawVersions) || rawVersions.length === 0) {
      ...
      // IMPORTANT: Use ?? (nullish coalescing) instead of || for all fields.
      // The || operator treats empty strings as falsy, so if the user CLEARS a field
      // (sets it to ""), || falls back to the old inv value. ?? only falls back on null/undefined.
      normalized = {
        ...inv,
        invoiceNumber: fixedNum,
        ...
        deliveryNote: latest.deliveryNote ?? inv.deliveryNote ?? '',
        paymentTerms: latest.paymentTerms ?? inv.paymentTerms ?? '',
        supplierRef: latest.supplierRef ?? inv.supplierRef ?? '',
        otherRef: latest.otherRef ?? inv.otherRef ?? '',
        buyerOrderNo: latest.buyerOrderNo ?? inv.buyerOrderNo ?? '',
        buyerOrderDate: latest.buyerOrderDate ?? inv.buyerOrderDate ?? '',
        despatchedThrough: latest.despatchedThrough ?? inv.despatchedThrough ?? '',
        destination: latest.destination ?? inv.destination ?? '',
        termsOfDelivery: latest.termsOfDelivery ?? inv.termsOfDelivery ?? '',
    ...
    setInvoiceHistory(prev => {
      ...
      if (existing) {
        ...
        upserted = { ...existing,
          ...
          companyBankName: invoiceData.companyBankName || '',
          companyAccNo: invoiceData.companyAccNo || '',
          companyBranch: invoiceData.companyBranch || '',
          companyIfsc: invoiceData.companyIfsc || '',
          companyVat: invoiceData.companyVat || '',
          companyCst: invoiceData.companyCst || '',
          companyPan: invoiceData.companyPan || '',
          deliveryNote: invoiceData.deliveryNote || '',
          paymentTerms: invoiceData.paymentTerms || '',
          supplierRef: invoiceData.supplierRef || '',
          otherRef: invoiceData.otherRef || '',
          buyerOrderNo: invoiceData.buyerOrderNo || '',
          buyerOrderDate: invoiceData.buyerOrderDate || '',
          despatchedThrough: invoiceData.despatchedThrough || '',
